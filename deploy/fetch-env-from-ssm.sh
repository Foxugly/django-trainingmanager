#!/usr/bin/env bash
# =============================================================================
# TrainingManager (wp.foxugly.com) — Fetch environment from AWS SSM into tmpfs.
#
# Run as root by trainingmanager-env-fetch.service (oneshot) at boot, BEFORE
# gunicorn starts. The written file lives in /run (tmpfs): it never touches
# disk, never lands in an EBS snapshot, and is re-fetched each boot.
#
# Source of truth = SSM /trainingmanager/prod/* (region eu-west-1), read with
# the EC2 instance role via IMDS — no AWS keys on disk. (The unit blanks
...
#   - exits non-zero on any failure -> trainingmanager-* (Requires=) won't start.
#
# §3.10: this runs as root, so the EXECUTION TARGET is the root-owned copy at
# /usr/local/sbin/trainingmanager-env-fetch.sh (installed by setup-server.sh).
# This file in the django tree is the versioned SOURCE, never run directly.
# =============================================================================
set -euo pipefail
...
# Parse JSON -> KEY=VALUE. Program read from the heredoc (python3 -), data read
# from the file passed as an argument — no clash between program/data on stdin.

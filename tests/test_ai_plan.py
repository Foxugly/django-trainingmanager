...
def test_POST_generate_events_as_team_manager_returns_200(auth_client_trainer, trainer_user, settings):
...
def test_POST_generate_events_updates_program_ai_fields(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    start = date(2026, 6, 1)
    end = date(2026, 6, 14)
...
def test_POST_generate_events_invalid_dates_returns_400(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    response = auth_client_trainer.post(
        f'/api/v1/programs/{program.pk}/generate-events/',
...
def test_POST_generate_events_range_too_long_returns_400(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    response = auth_client_trainer.post(
        f'/api/v1/programs/{program.pk}/generate-events/',
...
def test_POST_generate_events_ai_returns_invalid_date_returns_502(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    bad_event = {
        "name": "Out of range",
...
def test_POST_generate_events_ai_returns_empty_returns_502(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    with patch("tools.ai.Anthropic") as MockAnthropic:
        mock_client = MockAnthropic.return_value
...
def test_POST_generate_events_ai_does_not_call_tool_returns_502(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
...
def test_POST_generate_events_add_only_skips_duplicates(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    start = date(2026, 7, 1)
    end = date(2026, 7, 14)
    Event.objects.create(
        refer_program=program, name="Existing", date=start, color="#000000",
    )
...
def test_POST_generate_events_replace_deletes_existing_in_range(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    start = date(2026, 8, 1)
    end = date(2026, 8, 14)
...
def test_POST_generate_events_merge_adds_alongside_existing(auth_client_trainer, trainer_user, settings):
    settings.ANTHROPIC_API_KEY = "sk-ant-fake-test-key"
    program = _trainer_program(trainer_user)
    start = date(2026, 9, 1)
    end = date(2026, 9, 14)
